...
export function SidebarItem({ icon, text, active, alert, onClick}) {
  ...
  return (
    ...
    <button onClick={onClick} className="flex items-center w-full text-left">
      ...
      {!expanded && (
        <div className={`absolute left-full rounded-md px-2 py-1 ml-6 bg-[#63bbff] text-white-800 text-sm invisible opacity-20 -translate-x-3 transition-all group-hover:visible group-hover:opacity-100 group-hover:translate-x-0`}>
          {text}
        </div>
      )}
      </button>
