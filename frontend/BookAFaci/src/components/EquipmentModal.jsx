...
export default function EquipmentModal({ open, onClose, onSubmit, facilityName, equipments = [] }) {
  ...
  const updateEquipment = (id, delta) => {
                ...
                <div key={r._id || r.id} className="bg-[#F7FBFF] rounded-xl p-3 shadow-sm flex flex-col">
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => updateEquipment(r._id || r.id, -1)} className="w-9 h-9 rounded-md bg-gray-200 flex items-center justify-center">−</button>
                      <div className="w-10 text-center font-semibold">{selection[r._id || r.id] || 0}</div>
                      <button onClick={() => updateEquipment(r._id || r.id, 1)} className="w-9 h-9 rounded-md bg-[#007BDA] text-white flex items-center justify-center">+</button>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="w-1/3 p-6 border-l bg-gradient-to-b from-white to-gray-50 overflow-auto">
            ...
            <div className="space-y-2">
              {Object.entries(selection).filter(([, qty]) => qty > 0).map(([id, qty]) => {
                const r = equipments.find(x => (x._id || x.id) === id);
                return (
              ...
              {totalItems === 0 && <div className="text-sm text-gray-500 italic">No equipments selected</div>}
            </div>
            ...
            <div className="mt-6 space-y-2">
              <button onClick={handleContinue}
                className={`w-full py-3 rounded-lg font-bold bg-gradient-to-r from-[#346D9A] to-[#83C9FF] text-[#FFFFFF] hover:brightness-95`}>
                Book
              </button>
